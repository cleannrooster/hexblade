...
public class MagisterAI {
    ...
    private static <U extends Sensor<?>> SensorType<U> register(String p_26829_, Supplier<U> p_26830_) {
        return Registry.register(Registries.SENSOR_TYPE, Identifier.of(MOD_ID,p_26829_), new SensorType<>(p_26830_));
    }
    public MagisterAI() {
    }
    ...
    private static void initCoreActivity(Magister reaver, Brain<Magister> brain) {
        brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new LookAroundTask(45, 90), StrollTask.create(1.0F,true), TaskTriggerer.runIf(piglin -> !(reaver.isScout && reaver.nemesis != null), FindWalkTargetTask.create(0.75F)),   OpenDoorsTask.create(),  ForgetAngryAtTargetTask.create()));
    }

    private static void initIdleActivity(Magister reaver, Brain<Magister> brain) {
        brain.setTaskList(Activity.IDLE, 10, ImmutableList.of( StrollTask.create(1.0F,true), UpdateAttackTargetTask.create(MagisterAI::findNearestValidAttackTarget), makeRandomFollowTask(),   FindInteractionTargetTask.create(EntityType.PLAYER, 16)));
    }
